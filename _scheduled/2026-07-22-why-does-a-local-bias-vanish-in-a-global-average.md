---
layout: post
title: "Modelling MNAR data with population data #4 : Why does a local bias vanish in a global average?"
...
<!--
  ...
  NUMBERS, all from outputs/blog_figures_report.txt, and they match
  outputs/rung1_report.txt exactly:
    ...
    94% band width, Mayotte: 0.217pp for both. The correction moves the
    estimate, it does not widen it.

  PICK YOUR FRAMING OF THE HEADLINE NUMBER. Both are true and they differ:
    16.8%  the filled-in estimate falls by this much when corrected (delta / 1.252)
    20.2%  the filled-in estimate sits this much above the corrected (delta / 1.042)
